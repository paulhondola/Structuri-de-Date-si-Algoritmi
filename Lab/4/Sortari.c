...
int compare(tip_element el1, tip_element el2) { return el1.cheie - el2.cheie; }
...
void shell_sort(tip_element a[], int n) {
  ...
  for (i = 0; i < T; i++) {
    ...
    for (hCnt = 0; hCnt < H; hCnt++) { // insetion_sort pentru subtablou
...
void mergesort(tip_element A[], tip_element temp[], int left, int right) {
  ...
  int i1 = left;
  int i2 = mid + 1; // interclasare inapoi in A
  for (int index = left; index <= right; index++) {
    if (i1 == mid + 1)
      A[index] = temp[i2++]; // s-a epuizat secventa din stanga
    else if (i2 > right)
      A[index] = temp[i1++]; // s-a epuizat secventa din dreapta
...
void countingsort(tip_element a[], tip_element b[], int n) {
  int i, j;
  for (j = 1; j <= M - 1; j++)
    numar[j] = 0; // initializare tablou contorizare distributie elemente
  for (i = 1; i <= n; i++)
    numar[a[i - 1].cheie] =
        numar[a[i - 1].cheie] + 1; // numararea valorilor elementelor
  for (j = 1; j <= M - 1; j++)
    numar[j] = numar[j - 1] + numar[j]; // ajustare valori contoare
  for (i = n; i >= 1;
       i--) // mutare elemente pe poztia ordonata in tabloul temporar b
  {
...
unsigned biti(unsigned x, int k, int j) { return (x >> k) & ~(~0 << j); }
...
/*b - lungimea în biţi a cheii de sortat -1*/
{
  ...
  if ((dreapta > stanga) && (b >= 0)) {
    ...
    if (biti(a[dreapta], b, 1) == 0)
      j = j +
          1; /* daca ultimul bit testat este 0 se reface lungimea partitiei */
